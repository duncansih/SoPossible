...
- **Signature motifs**: a terminal-window chrome bar at the very top
  ...
  `$ run --tool`); slider thumbs as small glowing diamonds
...
### Phase 1 — Portal skeleton + tool(s) (in progress)
...
Color Palette Generator was scoped and its card shell was built, then
removed by request — dropped from the nav and the page. Not part of the
plan going forward unless re-scoped.
